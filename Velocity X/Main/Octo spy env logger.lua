...
do
	task.spawn(function() -- Instance1
		...
		-- ========== FILE LOGGING ==========
		local LOG_FILE = "OctoSpy_Log.txt"
		local fileLogEnabled = false

		local function writeToFile(text)
			if not text or text == "" then return end
			-- Try appendfile first (most executors)
			if getfenv().appendfile then
				pcall(function()
					getfenv().appendfile(LOG_FILE, text)
					fileLogEnabled = true
				end)
				return
			end
			-- Fallback to writefile with read+write
			if getfenv().writefile and getfenv().readfile then
				pcall(function()
					local existing = getfenv().readfile(LOG_FILE) or ""
					getfenv().writefile(LOG_FILE, existing .. text)
					fileLogEnabled = true
				end)
				return
			end
			-- Last resort: writefile without read (will overwrite)
			if getfenv().writefile then
				pcall(function()
					-- We don't have readfile, so we'll just use a separate function to append manually
					-- Actually, we can store a global variable to keep the log in memory and write on each call, but it's risky.
					-- Better to just use writefile and hope it doesn't overwrite.
					-- We'll store in a global table and write on flush, but that's complex. 
					-- We'll just warn the user.
					warn("File logging: writefile exists but readfile doesn't. Logs may overwrite.")
					getfenv().writefile(LOG_FILE, text)
					fileLogEnabled = true
				end)
				return
			end
		...
		-- File logging initial check
		if getfenv().appendfile or getfenv().writefile then
			-- write a header to the file
			local header = ("===== OctoSpy Log Started at %s =====\n"):format(os.date("%Y-%m-%d %H:%M:%S"))
			writeToFile(header)
			task.spawn(notification, "File logging enabled! Logs will be saved to " .. LOG_FILE, "File Logging")
		else
			task.spawn(notification, "File logging disabled: your executor does not support writefile/appendfile.", "File Logging")
		end
		...
		-- Helper to dump environment (global table) with depth limit
		local function dumpEnv()
			-- Use a copy of _G to avoid circular references causing recursion
			local seen = {}
			local function toStrLimited(tbl, depth)
				...
				for k, v in pairs(tbl) do
					if type(k) == "string" and k:match("^[%a_][%w_]*$") then
						parts[#parts+1] = k .. " = " .. toStrLimited(v, depth+1)
					else
						parts[#parts+1] = "[" .. toStrLimited(k, depth+1) .. "] = " .. toStrLimited(v, depth+1)
					end
				end
				seen[tbl] = nil
		...
		local function func(v, old, ...)
			...
			-- Write to file
			local fileEntry = ("[%s] %s\nRemote: %s\n%s\n\n"):format(os.date("%Y-%m-%d %H:%M:%S"), v.ClassName, v.Name, msg)
		...
		local function remoteEvent(v, ...)
			if spyActive and not ignore[v.Name] and not ignore[v] then
				...
				-- Write to file
				local fileEntry = ("[%s] %s (ClientEvent)\nRemote: %s\n%s\n\n"):format(os.date("%Y-%m-%d %H:%M:%S"), v.ClassName, v.Name, txt)
				writeToFile(fileEntry)
			end
		end
		local function bindableEvent(v, ...)
			if spyActive and logBindables and not ignore[v.Name] and not ignore[v] then
				...
				local content = "local args = "..args.."\n\n"..tostr(v)..":Fire(unpack(args)) -- Event"
				content = content .. "\n\n-- Environment:\n" .. dumpEnv()
				table.insert(insertionQueue, {log, "From", content, nil, v, args, "Fire"})

				-- Write to file
				local fileEntry = ("[%s] %s (BindableEvent)\nRemote: %s\n%s\n\n"):format(os.date("%Y-%m-%d %H:%M:%S"), v.ClassName, v.Name, content)
				writeToFile(fileEntry)
			end
		end
		...
		local fireServerHook, fireServer2Hook, invokeServerHook, fireHook, invokeHook = (newc(function(...)
			...
			if not ignore[self] and not ignore[self.Name] then
				...
				local content = "local args = "..args.."\n\n"..tostr(self)..":FireServer(unpack(args))"
				content = content .. "\n\n-- Environment:\n" .. dumpEnv()
				table.insert(insertionQueue, {log, "To", content, nil, self, args, "FireServer"})

				-- Write to file
				local fileEntry = ("[%s] %s (FireServer)\nRemote: %s\n%s\n\n"):format(os.date("%Y-%m-%d %H:%M:%S"), self.ClassName, self.Name, content)
				writeToFile(fileEntry)
			end
			...
			if not ignore[self] and not ignore[self.Name] then
				...
				local content = "local args = "..args.."\n\n"..tostr(self)..":FireServer(unpack(args)) -- Unreliable FireServer"
				content = content .. "\n\n-- Environment:\n" .. dumpEnv()
				table.insert(insertionQueue, {log, "To", content, nil, self, args, "FireServer"})

				-- Write to file
				local fileEntry = ("[%s] %s (Unreliable FireServer)\nRemote: %s\n%s\n\n"):format(os.date("%Y-%m-%d %H:%M:%S"), self.ClassName, self.Name, content)
				writeToFile(fileEntry)
			end
			...
			if not ignore[self] and not ignore[self.Name] then
				...
				local content = "local args = "..args.."\n\n"..tostr(self)..":InvokeServer(unpack(args))"
				content = content .. "\n\n-- Environment:\n" .. dumpEnv()
				local t = {log, "To", content, nil, self, args, "InvokeServer"}
				table.insert(insertionQueue, t)

				-- Write to file
				local fileEntry = ("[%s] %s (InvokeServer)\nRemote: %s\n%s\n\n"):format(os.date("%Y-%m-%d %H:%M:%S"), self.ClassName, self.Name, content)
				writeToFile(fileEntry)
